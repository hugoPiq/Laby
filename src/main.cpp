...
{
  cell->m_serial = true;
  *f << *cell; //attention cell est pointeur,la surcharge ne marche que sur la "valeur reelle"
  *f << cell->m_nb_neighb;
...
{
  ofstream f(file_name); //ouverture en ecriture (constructeur)
  f << "#Start:" << endl; 
  f << *maze.c0<< endl;
  f << "#End:" << endl; 
  f << *maze.cf << endl;
...
{
  map<pair<int,int>,Cell*> new_maze;
  ifstream f(file_name);
  string str1,str2,str3,str4;
  getline(f,str1); // #Start
  getline(f,str2); // Cell start
  getline(f,str3); // #End
  getline(f,str4); // Cell end
  pair<int,int> keyA {str2[1],str2[3]};
  pair<int,int> keyM {str4[1],str4[3]};
  ...
  //creation du maze
  Maze m;
   m.c0 = new_maze[keyA]; m.cf = new_maze[keyM];
   // manque l'ajout de voisin: aucune idées de comment faire, reparcous du fichier ?
  return m;
}
